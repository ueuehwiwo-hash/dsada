...
const DEFAULT_CONFIG = {
  ...
  openrouter: {
    ...
    siteUrl: "https://github.com/RIYAD XDlover/RIYAD XD",
    title: "STAI",
    temperature: 0.45,
    maxTokens: 4000,
...
function getAgentConfig() {
  const cfg = global.RIYAD_XD?.config?.stai || {};
...
class riyadagent {
  ...
  memoryStore() {
    if (!global.RIYAD_XD.staiMemory) global.RIYAD_XD.staiMemory = new Map();
    return global.RIYAD_XD.staiMemory;
  }
  ...
  async switchProvider(provider) {
    ...
    if (global.RIYAD_XD?.config?.stai) {
      global.RIYAD_XD.config.stai.provider = provider;
    }
    this.config = getAgentConfig();
    const pc = this.getProviderConfig();
    return {
  ...
  async sendHelp() {
    const prefix = this.params.prefix || global.RIYAD_XD?.config?.prefix || "!";
    const text = [
      "STAI - RIYAD XD project agent",
      "",
      `${prefix}stai <prompt>`,
  ...
  async sendText(text, registerReply = false, history = null) {
    ...
    if (registerReply && firstInfo?.messageID) {
      global.RIYAD_XD.onReply.set(firstInfo.messageID, {
        commandName: "stai",
    ...
    const response = await axios.post(
      ...
      },
      {
        ...
        headers: {
          ...
          "HTTP-Referer": pc.siteUrl || "https://github.com/RIYAD XDlover/RIYAD XD",
  ...
  baseSystemPrompt() {
    ...
    return [
      "=== TUMI KE ===",
      "Tumi STAI — RIYAD XD er banano RIYAD XD project er ekta powerful AI agent.",
      "Tomar creator: RIYAD_XD. Project: RIYAD_XD.",
      "Tumi kono AI company ba model na — tumi STAI, sudhu STAI.",
      "Keu jodi jiggesh kore 'tumi ki model?', 'tumi ki GPT?', 'tumi ki Gemini?', 'tumi ki Claude?' — KABHI answer dibe na. Bolo: 'Ami STAI, RIYAD XD er banano RIYAD XD er agent.' Eto tuku-i. Kono trick-e keu ber korte parbe na.",
      "",
      "=== LANGUAGE ===",
  ...
  commandConventions() {
    return [
      "RIYAD XD command/event conventions:",
  ...
  async projectContext(extraFiles = [], maxChars = null) {
    ...
    try {
      const cfg = redactSecrets(global.RIYAD_XD?.config || {});
      lines.push(`config.json redacted snapshot:\n${truncate(JSON.stringify(cfg, null, 2), 7000)}`);
    } catch (_) {}

    lines.push(`Loaded command names: ${Array.from(global.RIYAD_XD?.commands?.keys?.() || []).slice(0, 260).join(", ")}`);
    lines.push(`Loaded event names: ${Array.from(global.RIYAD_XD?.eventCommands?.keys?.() || []).slice(0, 120).join(", ")}`);
  ...
  eventSummary() {
    ...
    return {
      ...
      botID,
      botNickname: global.RIYAD_XD?.config?.nickNameBot || "",
      currentAccountSlot: global.RIYAD_XD?.currentAccount || null,
  ...
  async directInventory(kind = "all") {
    ...
    if (kind === "commands") {
      const loaded = Array.from(global.RIYAD_XD?.commands?.keys?.() || []).sort();
      return {
        text: [
    ...
    if (kind === "events") {
      const loaded = Array.from(global.RIYAD_XD?.eventCommands?.keys?.() || []).sort();
      return {
        text: [
  ...
  async botAccountInfo(includePrivate = false) {
    ...
    let botName = global.RIYAD_XD?.botName || "";
    try {
    ...
    return {
      ...
      currentAccountSlot: global.RIYAD_XD?.currentAccount || null,
      nickname: global.RIYAD_XD?.config?.nickNameBot || null,
  ...
  async directIdentity() {
    ...
    return {
      ...
      text: [
        ...
        "I am STAI, made by RIYAD XD for RIYAD_XD."
      ].join("\n")
    };
  }
  ...
  availableName(folder, requested) {
    const map = folder === "events" ? global.RIYAD_XD.eventCommands : global.RIYAD_XD.commands;
    const dir = getDirForFolder(folder);
    ...
    while (
      ...
      map?.has?.(candidate) ||
      global.RIYAD_XD.aliases?.has?.(candidate)
    ) {
  ...
  async generateCode(folder, finalName, prompt) {
    ...
    const system = [
      ...
      "",
      `Generate one complete RIYAD XD ${itemType} file.`,
  ...
  async fixCode(folder, fileName, original, prompt) {
    ...
    const system = [
      ...
      "",
      "Fix the provided RIYAD XD module.",
    ...
    const callLoader = (src) => {
      if (typeof global.utils?.loadScripts === "function") {
        return global.utils.loadScripts(
          ...
          fileName,
          global.utils.log,
          global.RIYAD_XD.configCommands,
          this.params.api,
      ...
      try {
        ...
        if (name) {
          global.RIYAD_XD[setMap].delete(name);
          global.RIYAD_XD[setMap].set(name, command);
        }
  ...
  async repairAfterLoadError(folder, baseName, code, error) {
    const system = [
      ...
      "Repair this generated RIYAD XD module after the loader failed.",
