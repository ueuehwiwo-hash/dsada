...
if (!global.RIYAD_XD) global.RIYAD_XD = {};
...
const { callbackListenTime, storage5Message } = global.RIYAD_XD;
...
global.RIYAD_XD.e2eeFullyReady = false;
global.RIYAD_XD.pendingE2eeRestartNotifications = fs.existsSync(restartNoticePath)
  ? (global.RIYAD_XD.pendingE2eeRestartNotifications || [])
  : [];
...
global.RIYAD_XD.sendPendingE2eeRestartNotifications = async function sendPendingE2eeRestartNotifications(api) {
  if (global.RIYAD_XD.e2eeRestartNotificationSending)
    return;
  global.RIYAD_XD.e2eeRestartNotificationSending = true;

  try {
    const pending = global.RIYAD_XD.pendingE2eeRestartNotifications || [];
    ...
    global.RIYAD_XD.e2eeRestartNotificationSending = false;
  }
};
...
async function getAppStateToLogin(loginWithEmail, useSecondaryAccount = false) {
  let appState = [];
  const twoIdMode = global.RIYAD_XD.config.twoIdMode;
...
function stopListening(keyListen) {
  ...
  return new Promise((resolve) => {
    global.RIYAD_XD.fcaApi.stopListening?.(() => {
...
async function startBot(loginWithEmail, useSecondaryAccount = false) {
  ...
  if (twoIdMode && twoIdMode.enable && useSecondaryAccount) {
    ...
    global.RIYAD_XD.currentAccount = "secondary";
  } else {
    // Always default to primary
    useSecondaryAccount = false;
    global.RIYAD_XD.currentAccount = "primary";
  }
  ...
  if (global.RIYAD_XD.Listening)
    await stopListening();

  log.info("LOGIN FACEBOOK", getText('login', 'currentlyLogged'));

  // Use the twoIdMode variable already declared above
  useSecondaryAccount = twoIdMode && twoIdMode.enable && global.RIYAD_XD.currentAccount === "secondary";
  ...
  (function loginBot(appState) {
    global.RIYAD_XD.commands = new Map();
    global.RIYAD_XD.eventCommands = new Map();
    global.RIYAD_XD.aliases = new Map();
    global.RIYAD_XD.onChat = [];
    global.RIYAD_XD.onEvent = [];
    global.RIYAD_XD.onReply = new Map();
    global.RIYAD_XD.onReaction = new Map();
    ...
    login({ appState }, global.RIYAD_XD.config.optionsFca, async function (error, api) {
      ...
      if (error) {
        ...
        if (isCookieError && !global.RIYAD_XD.retryWithBotaccAttempted) {
          ...
          global.RIYAD_XD.retryWithBotaccAttempted = true;
          ...
          if (botAccountConfig.email && botAccountConfig.password) {
            try {
              ...
              delete global.RIYAD_XD.retryWithBotaccAttempted;
        ...
        if (global.RIYAD_XD.config.autoRestartWhenListenMqttError) {
          log.info("AUTO RESTART", "Restarting...");
          process.exit(2);
        }
        else {
          // —————————— CHECK DASHBOARD —————————— //
          if (global.RIYAD_XD.config.dashBoard?.enable == true) {
            try {
      ...
      global.RIYAD_XD.fcaApi = api;
      global.RIYAD_XD.botID = api.getCurrentUserID();
      log.info("LOGIN FACEBOOK", getText('login', 'loginSuccess'));

      // ─── E2EE connect with status logging ──────────────────────────────────
      if (global.RIYAD_XD.config.e2ee && global.RIYAD_XD.config.e2ee.enable === true) {
        if (typeof api.connectE2EE === 'function') {
          ...
          api.connectE2EE(function (err, evt) {
            ...
            if (evt.type === "e2ee_connected")
              ...
              global.RIYAD_XD.e2eeFullyReady = true;
              log.info("E2EE", "🟢 E2EE fully ready — encrypted messaging active");
              global.RIYAD_XD.sendPendingE2eeRestartNotifications(api).catch(function (err) {
              ...
              try {
                const _bsn = global.RIYAD_XD.config.botStartupNotification;
      ...
      log.info("PREFIX", global.RIYAD_XD.config.prefix);
      log.info("LANGUAGE", "en");
      log.info("BOT NICK NAME", global.RIYAD_XD.config.nickNameBot || "RIYAD XD");

      // Display update enforcement status
      const updateAvailable = global.updateAvailable || global.RIYAD_XD.updateAvailable;
      const updateRefuseUntil = global.updateRefuseUntil || global.RIYAD_XD.updateRefuseUntil;
      ...
      if (global.RIYAD_XD.config.autoRefreshFbstate == true) {
        changeFbStateByCode = true;
        try {
      ...
      global.RIYAD_XD.dashboardContext = {
        api,
        threadModel,
        userModel,
        dashBoardModel,
      ...
      // ———————————————————— BIO UPDATE ————————————————————— //
      const { bioUpdate } = global.RIYAD_XD.config;
      ...
      if (global.RIYAD_XD.config.autoLoadScripts?.enable == true) {
        const ignoreCmds = global.RIYAD_XD.config.autoLoadScripts.ignoreCmds?.replace(/[ ,]+/g, ' ').trim().split(' ') || [];
        const ignoreEvents = global.RIYAD_XD.config.autoLoadScripts.ignoreEvents?.replace(/[ ,]+/g, ' ').trim().split(' ') || [];
          ...
          if (filename.endsWith('.js')) {
            ...
            if ((event == 'change' || event == 'rename') && existsSync(`${process.cwd()}/scripts/cmds/${filename}`)) {
              try {
                ...
                const infoLoad = global.utils.loadScripts("cmds", filename, log, global.RIYAD_XD.configCommands, api, threadModel, userModel, dashBoardModel, globalModel, threadsData, usersData, dashBoardData, globalData);
          ...
          if (filename.endsWith('.js')) {
            ...
            if ((event == 'change' || event == 'rename') && existsSync(`${process.cwd()}/scripts/events/${filename}`)) {
              try {
                ...
                const infoLoad = global.utils.loadScripts("events", filename, log, global.RIYAD_XD.configCommands, api, threadModel, userModel, dashBoardModel, globalModel, threadsData, usersData, dashBoardData, globalData);
                if (infoLoad.status == "success")
      ...
      // ——————————————————— DASHBOARD ——————————————————— //
      if (global.RIYAD_XD.config.dashBoard?.enable == true && dashBoardIsRunning == false) {
        console.log("");
      ...
      let i = 0;
      const adminBot = global.RIYAD_XD.config.adminBot
      ...
      log.master("LOAD TIME", `${convertTime(Date.now() - global.RIYAD_XD.startTime)}`);
      ...
      const configToSave = { ...global.RIYAD_XD.config };
      configToSave.adminBot = adminBot;
      writeFileSync(global.client.dirConfig, JSON.stringify(configToSave, null, 2));
      writeFileSync(global.client.dirConfigCommands, JSON.stringify(global.RIYAD_XD.configCommands, null, 2));

      // ——————————————————— START MESSAGE CHECKER ———————————————————— //
      try {
        const cleanupMessageChecker = global.utils.startMessageChecker(api, global.RIYAD_XD.config);
        global.RIYAD_XD.messageCheckerCleanup = cleanupMessageChecker;
      } catch (err) {
        // Silent fail
      }

      // ——————————————————————————————————————————————————— //
      const { restartListenMqtt } = global.RIYAD_XD.config;
      ...
      async function callBackListen(error, event) {
            ...
            if (intervalCheckLiveCookieAndRelogin == false) {
              ...
              const interval = setInterval(async () => {
                ...
                if (cookieIsLive) {
                  ...
                  global.RIYAD_XD.Listening = api.listenMqtt(createCallBackListen(keyListen));
                }
              }, 5000);
            }
            return;
        ...
        global.responseUptimeCurrent = responseUptimeSuccess;
        global.statusAccountBot = 'good';
        const configLog = global.RIYAD_XD.config.logEvents;
        ...
        if (
          global.RIYAD_XD.config.whiteListMode?.enable == true
          && global.RIYAD_XD.config.whiteListModeThread?.enable == true
          // admin
          && !isUserAdmin
        ) {
          if (
            effectiveUserID && !global.RIYAD_XD.config.whiteListMode.whiteListIds.includes(effectiveUserID)
            && event.threadID && !global.RIYAD_XD.config.whiteListModeThread.whiteListThreadIds.includes(event.threadID)
            // admin
            && !isUserAdmin
          )
            return;
      ...
      global.RIYAD_XD.Listening = api.listenMqtt(async (error, event) => {
        ...
        if (error && error.error === "Not logged in" && error.res && error.res.jsmods) {
          ...
          if (hasCheckpoint && !checkpointDetected) {
            ...
            try {
              ...
              global.RIYAD_XD.Listening = api.listenMqtt(createCallBackListen());
              return;
            } catch (err) {
      ...
      global.RIYAD_XD.callBackListen = callBackListen;
      // ——————————————————— UPTIME ——————————————————— //
      if (global.RIYAD_XD.config.serverUptime.enable == true && !global.RIYAD_XD.config.dashBoard?.enable && !global.serverUptimeRunning) {
        ...
        const PORT = global.RIYAD_XD.config.dashBoard?.port || (!isNaN(global.RIYAD_XD.config.serverUptime.port) && global.RIYAD_XD.config.serverUptime.port) || 3001;
        ...
        try {
          ...
          if (global.RIYAD_XD.config.serverUptime.socket?.enable == true)
  ...
  if (global.RIYAD_XD.config.autoReloginWhenChangeAccount) {
    setTimeout(function () {
...
global.RIYAD_XD.reLoginBot = startBot;
startBot();
