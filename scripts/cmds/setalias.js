	...
	onStart: async function ({ message, event, args, threadsData, globalData, role, getLang }) {
		...
		switch (args[0]) {
			case "add": {
				...
				if (!global.RIYAD_XD.commands.has(commandName))
				...
				if (args[3] == '-g') {
					if (role > 1) {
						...
						if (global.RIYAD_XD.aliases.has(alias))
							return message.reply(getLang("aliasExist", alias, global.RIYAD_XD.aliases.get(alias)));
						...
						global.RIYAD_XD.aliases.set(alias, commandName);
				...
				if (global.RIYAD_XD.commands.get(alias))
			...
			case "rm": {
				...
				if (args[3] == '-g') {
					if (role > 1) {
						...
						await globalData.set('setalias', globalAliasesData, 'data');
						global.RIYAD_XD.aliases.delete(alias);
